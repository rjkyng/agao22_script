...
\begin{align*}
  ...
  \st\quad  & \BB\ff = \dd
\end{align*}
where $\calE:S\subseteq\R^m\to\R$ is a convex function and $\BB\in\R^{n\times m}$. Wlog, let $\dd=\matzero$, since otherwise we can equivalently deal the following program with $\BB\ff_0 = \dd$,
\begin{align*}
...
What is a Newton step $\ddelta^*$ in a linearly constrained optimization program? $\ddelta^*$ should be a minimizer of
\begin{align*}
