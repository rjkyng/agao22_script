...
In the following we will denote by $\alpha^* = \energy(\yy^*)$ the optimal value of the this program where $\yy^*$ is an minimizer.
...
Let's come back our earlier discussion of parallel gradients and the
KKT conditions.
...
strong duality, now is implied by the KKT conditions. 
\begin{theorem}
   Consider a convex program \eqref{eq:primalprob} with an open domain
  set $S$ whose dual satisfies
   strong duality.
