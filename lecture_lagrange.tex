...
$\aa_j^{\top}$, i.e. gradients of the linear constraint, and of
gradients $\grad c_i(\yy^*)^{\top}$ of tight inequality constraints,
...
Nonetheless, under some mild conditions, it is indeed true that these
conditions must hold at any optimal solution.
...
\begin{definition}[The Karush-Kuhn-Tucker (KKT) Conditions]
  ...
  \begin{itemize}
  \item $\AA\yy = \bb$ and $\cc(\yy) \leq 0$ \hfill (primal feasible)
  \item $\ss \geq 0$ \hfill (dual feasible)
  \item $\grad_{\yy}\calE(\yy) + \AA^\trp\xx +
    \ss^\trp\grad\cc(\yy) = \matzero$ \hfill
    (Equation~\eqref{eq:compSlackIntro} again)
...
A (primal) problem as defined in \eqref{eq:primalprob} with $S$ being
an open set fulfills
